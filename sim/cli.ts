...
async function main(): Promise<void> {
  ...
  console.log(
    `refused_good=${result.summary.meanRefusedGoodMoveRate.toFixed(3)} override=${result.summary.meanOverrideRate.toFixed(3)} win=${result.summary.meanWinScore.toFixed(1)} unjustified_trauma=${result.summary.meanUnjustifiedTrauma.toFixed(2)} leadership_index=${result.summary.meanLeadershipIndex.toFixed(2)} emptied_chairs=${result.summary.meanEmptiedChairs.toFixed(2)} emptied_chairs_score=${result.summary.meanEmptiedChairsScore.toFixed(2)} mean_trust_final=${result.summary.meanTrustFinal.toFixed(2)} trust_delta=${result.summary.meanTrustDelta.toFixed(2)}`,
  );
