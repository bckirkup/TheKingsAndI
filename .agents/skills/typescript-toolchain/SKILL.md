...
- **Nightly / dispatch:** `.github/workflows/nightly.yml` — `pnpm test:heavy`,
  the 20-match two-leader smoke, Lozza N≈100 calibration as parallel
  tyrannical / supportive / sweep jobs (supportive needs a long timeout),
  and Stockfish only via `workflow_dispatch` with an explicit match budget.
