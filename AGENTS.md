...
| `docs/adr/0073-hope-courage-and-the-closing-debrief.md` | **D195–D197 ruled; D199 ruled and wired in the closing debrief.** Courage is action against the actor's own expected cost, normalized as asked-risk-relative `min(1, margin / asked)`. It is debrief-only: no gauge, stated reason, facilitator console, leader policy, or adaptive-player observation. The campaign reading is a mean over acts, never a sum; named incidents appear only in the closing debrief. **Hope is still not wired** — it needs a per-piece object list (object + prospect + credence, extinguished by unreachability), and **D198** (what destroying a hope object costs) remains open |
