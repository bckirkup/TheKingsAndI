...
| `docs/adr/0070-graded-witness-loss-and-standing-price.md` | **D170 and D174 ruled, mechanism inert:** each witness prices the overridden piece through its own affinity and class prestige, but standing can only raise the witness charge; the witness multiplier and standing-price defaults preserve today's behavior. D176 owns the joint live magnitudes and the free-insistence sweep gate |
...
| `docs/calibration/2026-08-29-the-graded-witness-surface.md` | **Read before choosing any D176 magnitude:** the joint D170/D174 surface on the two conditions that still carry free insistence. Grading the witness *down* removes free insistence over a campaign (0.3411 → 0.0000) even though it widens the per-state truncation band — depletion dominates truncation, so the safe direction for `OVERRIDE_WITNESS_BENEV_MULTIPLIER_PERMILLE` is down, not up. The standing price sits on the same axis and re-opens the floor at `8000`; no behavioural metric moves anywhere except at multiplier `2000`, which is worse. Recommended cell: `500` / `2000` |
