...
| `docs/adr/0078-the-uncarried-emotions.md` | **D208, D209, D211, D212, D213, and D214 wired inert; D210 recognition is partly wired; D216 partly wired for recognition, inert; D215 and D217 remain open.** Bitterness is an optional terminal-only carrier with rupture-floor and captive-decay recognition, inert repair/regard/morning-lift discounts, and match-boundary decay. Spite is a match-local, terminal-only recognition fold over existing grievance and act events, with both floors disabled by default. Grief carries clamped deterministic mourning and terminal naming only. Shame is a terminal-only, default-inert reading that reuses ADR 0070 witnesses and scales only the target's own unvindicated-override loss. Guilt is a direct-link, terminal-only fold over desertion cascades and floor-gated peer-safety spend annotations, with zero defaults. Envy is a seminar-only, terminal-only recognition fold over draft settlements, inert at its zero floor. Panic is a terminal-only recognition fold over per-ply capture-risk and King-danger events, inert at its zero floor. Gratitude remains a terminal-only ransom recognition fold; live carriers, thresholds, magnitudes, and pricing remain open |
