...
| `docs/adr/0071-captivity-and-the-exchange.md` | **D177/D181/D183 wired in the seminar path only:** captures may remain as a private **fengr**, draft **hlutr** clearing prices pay piece cash (the exact ruling is “Pieces first get paid during the draft”), and the cycle-purse ransom round credits captors while exposing only who came home. Hold and benevolence-decay knobs are off/inert by default; campaign and season remain disabled. The v1 redemption ordering is harness policy, not doctrine. D182 self-appraisal magnitude/registration, D184, D185, and captor affinity remain open; weregild was considered and rejected because ransom prices a return, not a life, and wælreaf is the foreclosed spoils alternative |
...
| `docs/adr/0073-hope-courage-and-the-closing-debrief.md` | **D195–D197 ruled; D199 wired; promotion-hope v1 wired in the closing debrief.** Courage is action against the actor's own expected cost, normalized as asked-risk-relative `min(1, margin / asked)`. Hope transitions are naming-only, debrief-only events: realization, unreachability at prospect `0`, capture, and rekindling; no formation threshold or cost is introduced. **D198** (what destroying a hope object costs) remains open |
...
| `docs/adr/0078-the-uncarried-emotions.md` | **The uncarried emotions located; D208–D217 all open, nothing wired.** A survey ADR mapping bitterness, spite, gratitude, grief, shame, guilt, envy, pride, panic, and relief/awe/loneliness to their exact seams (carrier field or pure fold, trigger the log witnesses, decay, debrief naming), each inert until its own priced ruling; the house disciplines (D203 quarantine, ADR 0073 debrief-only naming, the ε measurement discipline, exploit-tier re-runs for any play change) bind every entry |
