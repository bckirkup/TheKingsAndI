...
| `docs/calibration/2026-09-20-does-the-engine-matter.md` | **Read before quoting any fake-engine census as evidence about the ability channel, or before choosing an engine (D46):** paired seeds 7/41 × tyrannical/supportive × {fake, Lozza-4, Stockfish-16}, 20 matches each, `random` opponent. The fake engine leaves `τ_abil` at 0.6–6 in every campaign (the D19 ability channel has been dead in every Phase A–C census); Lozza-4 matches Stockfish on the tyrant's ability credence (94–95) but not the kind room's (14–19 against 84), where its refusal is 0.51–0.57 against Stockfish's 0.06. Stockfish's tyrant is a different character (quiet-quit ×5, desertion ×2–8, trust −4 not −28, no degeneracy detector fires) and that gap is **not** a depth-cap effect (Lozza-8 does not move it); the cap explains about half the kind-room gap at ~4× the cost. Stockfish seed-41 supportive **collapsed** (trust −3.5, refusal 0.55, LI 26.9 after one catastrophic opening match) — one seed, reproduce before ruling. Stockfish is 7–34× Lozza-4 per match; a Stockfish census is not recommended. Harness fix: the Stockfish adapter gets its own 4096 info-line ceiling (Lozza keeps `runaway-512`). **Its cap and collapse questions are answered by the 09-21 follow-up below** |
| `docs/calibration/2026-09-21-lozza-at-depth-and-the-bistable-kind-room.md` | **Read before raising Lozza's depth cap, choosing an engine (D46), or quoting a supportive campaign under a real engine:** Lozza at `--depth-cap=8` (seed 41) and `--depth-cap=16` (seed 7, uncapped-equivalent) plus Stockfish-16 supportive on seeds 3/11/19/23/29, 20 matches each. The Lozza–Stockfish gap is an **engine gap, not a depth gap, in both rooms**: Lozza-16 reproduces the Lozza tyrant exactly (quiet-quit 0.022, trust −27, LI 18) at Stockfish cost, and the kind room's `τ_abil` is non-monotone in cap (19 → 38 → 17 for 4/8/16 against Stockfish's 84) — the 09-20 "cap explains half the kind-room gap" reading is superseded; keep `--depth-cap=4`. The Stockfish seed-41 collapse **reproduces**: 2 of 7 supportive seeds (29, 41) land in a tyrant-shaped basin (trust −29/−3.5, LI 16/27, refusal 0.5, desertion 0.45/0.30) (seed 29 with `τ_abil` and `τ_benev` ≤ 10 from quartile 1 and no recovery); the other five cluster at trust 94.5–97.2, LI 63.6–65.0, and no fake or Lozza cell at any cap ever collapses. The kind room is bistable under a real engine and the basin is selected in the first matches — a live D-question. Proposed next: per-match transcripts of seeds 29/41, the same seeds at `--opponent=tyrannical`, and a direct Lozza-vs-Stockfish centipawn-scale comparison before any third engine. No AWS campaign warranted; Stockfish stays GPL-excluded |
| `docs/calibration/2026-09-21-the-ply-seven-tie-break.md` | **Read before quoting the Stockfish kind-room "collapse" as a psychology finding or a collapse rate:** the seed 29/41 collapse is a **match-1 room dismissal** (ply 41/45, 319/480 refusals, 4–5 desertions in that one game); matches 2–20 start at inherited trust −4…−28 and are re-dismissed within 1–12 plies, so they are King-command games, not supportive evidence. Decision journals of seeds 7 and 41 show both open a4 d4 f4 and diverge on the **ply-7 seeded tie-break**: e4 (seed 7, zero refusals all match) against h4 (seed 41, one refusal, then the supportive policy — chess-blind, no refused-move memory, never overrides — re-orders refused moves until every legal move is refused each ply and the implicit override finishes the room). The same seeds at `--opponent=tyrannical` **do not collapse** (0/40 dismissals, LI 63–64), so collapse is a trajectory property, not a seed property — with the caveat that the opponent shares the leader's PRNG stream, so the archetype is not isolated. The engine supplies the trigger (a depth-16 view flags a flank push as "beyond the limit"; fake/Lozza never do); the spiral belongs to the leader policy — a harness defect to fix or name before any collapse rate is quoted. New in s41-vs-tyrannical: `τ_abil` 37 → 5 while `τ_benev` stays 82–92 and trust ends 84–100 (disengaged-but-loyal; one campaign, hypothesis). Proposed next: refused-move memory for the supportive pseudo-player, a separate opponent PRNG stream, and the centipawn-scale comparison. No AWS campaign warranted |
| `docs/calibration/2026-09-21-the-permissive-engine-survey.md` | **Read before implementing MultiPV for Avalanche or shipping a permissive engine:** the survey measured native UCI Avalanche 4.0.0 but no MultiPV, and verified MultiPV-8 in MIT Caissa and Zahak. Caissa is the Stockfish-class candidate; its native-spawn adapter and the Caissa-16 grid are the next stage. The Caissa-Nets `.pnn` has no checked-in LICENSE/NOTICE, so its redistribution terms are an open legal question before enterprise shipment |
