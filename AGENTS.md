...
| `docs/adr/0078-three-trials-before-anyone-loves-it.md` | **The plan after the harness: one journal, three kinds of hand.** The GUI is a journal writer and the screen is the observation projection; the order is journal-in-app (with the harness engine in the browser) → model personas (honest, merchant, vicious, bored, disengaged) with a computed containment envelope → the surface (legibility, four data packs, access) → paid strangers then a pilot cohort → the facilitator audit, dashboard, and a model facilitator judged by the same audit. Opens D208–D213; nothing wired |
