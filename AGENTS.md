...
| `docs/adr/0065-the-confidence-and-the-culture.md` | **D168 ruled, D169 open, nothing wired:** a private word that may not be kept. The channel must exist, but good news makes poor gossip, benevolence reads as favoritism, and no act in it is free. Records that rumor diffusion runs only in the desertion cascade and that `leaderAppraisal` is a write-only field |
