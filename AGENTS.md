...
| `docs/adr/0072-retirement-and-grace.md` | **D186 and D187 ruled — not wired:** carrying campaign state made trauma accrue for the first time, and no trauma transition lowers an in-range `B_i` (normalization only clamps malformed or out-of-range values), so the campaign path ratchets to the `clampTrauma` ceiling and ends no career (redeemer: 12 → 195 desertions, 16 → 4 identities ever surviving a match). Trauma at the threshold permanently retires a non-King identity **on every path**, and trauma relief exists as **grace**: unearned, unpurchasable, with no leader-controlled input in the term (not standing, purse, credence, style, result, or whether the commander ransomed the piece), no credit to the leader, drawn from the seeded PRNG at a match boundary, never anticipated by a piece (ADR 0011), and falling on both armies. Relief in `B_i` is flat; only its *registration* varies with expectation (D182). **D188** (rate and magnitude, gated on a cruel style not out-performing a kind one through grace) and **D189** (what fills a retired identity's square) are open |
