...
| `docs/adr/0066-the-floor-under-the-curdle.md` | **D167 ruled, wired inert:** the witness broadcast stays (being rough on one piece curdles the room), but the saturation beneath it — a flat cliff, an identical witness hit, and a ledger clamped at 100 — is the defect that makes later overrides free. Witness cliff input, a proportional (permille) cliff, and a rupture-debt ceiling are separable knobs whose defaults reproduce today's behaviour; the live magnitudes are one response surface with D166 and must not be chosen singly. D170 (status-priced overrides) is deliberately left open |
