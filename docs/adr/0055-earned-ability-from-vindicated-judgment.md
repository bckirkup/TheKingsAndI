...
pieces stake judgments and are graded. An objector is graded only when
`actorChallenged` records that the leader overrode her refusal; an accepted
refusal has no played move or audit truth from which to learn.
...
An `ABILITY_GRADE` event records the piece, ply, polarity, and applied nonzero
delta. Campaign ability spread metrics fold the resulting state, while the pool
ranks fielding ability relative to the origin role's starting ability so
promotion does not preserve a permanent birth-role handicap.
...
`actorChallenged` is set only when a refusal is overridden, so an objector learns
from a judgment only when her objection is settled by force; an accepted refusal
has no played move or audit truth to grade. Near-refusals who comply are graded
from the same vindication truth. Once the magnitude is nonzero,
