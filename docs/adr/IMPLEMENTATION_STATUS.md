...
| 0063 | Two duties: coverage and containment | **Accepted** for the duties and the D159–D163 schema answers; D164 (widening the span) open and owner-owned | **Not wired** — the coverage duty is measured today only by running the nine styles by hand (`sim/cli.ts:36-56`, `sim/degeneracy.ts`), evidence in `docs/calibration/2026-08-26-npc-coverage-and-the-envelope.md`; no envelope/containment metric, journal, or `disengage` option exists |
