...
| 0065 | The confidence and the culture: a private word that may not be kept | **Accepted in direction** — D168 ruled 2026-08-28 (the channel must exist; good news makes poor gossip; benevolence reads as favoritism; nothing is free), **D169 still open** (may `leaderAppraisal` be read) and nothing ships until it is ruled | **Not wired** — no private communication, discretion ladder, leak event, favoritism term, per-kind transmission rate, or culture seed exists, and no magnitudes are chosen. The transmission graph it would reuse is half-live: `diffuseRumor`/`applyRumorDiffusion` (`src/psychology/belief.ts:14-53`) are shipped and knobbed (`src/psychology/config.ts:143-144`) but have a single production call site in the desertion cascade (`src/psychology/cascade.ts:169`), and `rumor.leaderAppraisal` is **write-only** — no verdict, utility, desertion, or counsel term reads it |
