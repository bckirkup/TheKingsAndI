...
| 0061 | The order of work | **Proposed** — sequencing only; supersedes ADR 0054 §6 staging from slice 4 onward | **Partly wired** — scarcity step 1 is wired at zero through shared composition and levy controls (`src/orchestration/squadFielding.ts:37-42,234-320`); later steps remain unimplemented and the three commendation fidelity defects are still live (`src/persistence/commendations.ts:108-214`) |
