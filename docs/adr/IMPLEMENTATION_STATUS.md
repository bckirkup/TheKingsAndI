...
| 0079 | Three trials before anyone loves it: the surface, the stranger, and the room | **Accepted** — a plan; opens D218–D223, of which D218 (gate passed for act one + seminar harness only) and D221 (model facilitator is a stand-in fallback outside ADR 0004), and D219 (the GUI is a journal writer over the existing seam; screen shows only the `Observation`) are ruled; D220, D222, D223 (playtest upload, disengagement, pack order) remain open | **Not wired** — `src/` contains no journal writer, the interactive match path still constructs a fake engine (`src/app/README.md`), no model agent or fork exists, and no facilitator audit, dashboard, or host is in tree |
