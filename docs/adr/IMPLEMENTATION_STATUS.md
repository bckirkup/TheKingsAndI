...
| 0065 | The confidence and the culture: a private word that may not be kept | **Proposed** — D168 (does the channel exist) and D169 (may `leaderAppraisal` be read) are owner rulings; nothing ships until both are ruled | **Not wired** — no private communication, discretion ladder, leak event, or culture seed exists. The transmission graph it would reuse is half-live: `diffuseRumor`/`applyRumorDiffusion` (`src/psychology/belief.ts:14-53`) are shipped and knobbed (`src/psychology/config.ts:143-144`) but have a single production call site in the desertion cascade (`src/psychology/cascade.ts:169`), and `rumor.leaderAppraisal` is **write-only** — no verdict, utility, desertion, or counsel term reads it |
