...
| 0079 | Three trials before anyone loves it: the surface, the stranger, and the room | **Proposed** — a plan only; opens D218–D223 (the ADR 0061 step-7 gate, the GUI as journal writer, playtest upload, the model facilitator, disengagement, pack order) | **Not wired** — `src/` contains no journal writer, the interactive match path still constructs a fake engine (`src/app/README.md`), no model agent or fork exists, and no facilitator audit, dashboard, or host is in tree |
