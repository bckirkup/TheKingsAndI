...
| 0071 | Captivity and the exchange: being taken is being held | **Accepted** for D177/D181/D183; seminar-only wiring dated 2026-08-30. The exact ruling is “Pieces first get paid during the draft”; terminal exchange hope is now folded in the seminar path. D182 price-side magnitude is ruled and wired inert; live registration, D178–D180, D184, and D185 remain open | **Partly wired** — seminar pool folding enables the opt-in captive hold, persistent piece cash, pre-draft ransom, captor credit, inert-by-default benevolence decay, and terminal exchange-hope incidents. Campaign and season callers do not pass captivity metadata and remain unchanged; unransomed captives never enter draft lots. |
...
| 0078 | The uncarried emotions: a location survey | **D208, D209, D211, D212, D213, and D214 wired inert; D210 partly wired — gratitude recognition fold; D215 partly wired for price appraisal, inert; D216 and D217 partly wired for recognition, inert** | **D208 bitterness, D209 spite recognition, D211 grief, D212 shame, D213 guilt, D214 envy, D216 panic, and D217 relief/awe/loneliness are terminal-only and inert by default; D210 gratitude recognition is partly wired; D215 price appraisal is partly wired and inert**; live carriers, thresholds, magnitudes, and pricing remain open |
| 0079 | Three trials before anyone loves it: the surface, the bots, and the room | **Accepted** — a plan; opens D218–D223, of which D218 (gate passed for act one + seminar harness only) and D221 (model facilitator is a stand-in fallback outside ADR 0004), and D219 (the GUI is a journal writer over the existing seam; screen shows only the `Observation`) D222 (disengagement = choices indistinguishable from uniform over the option set, measured from the journal), and D223 (packs: military, medieval, corporate, classic/purist, in that order) are ruled; D220 (playtest upload) closed as moot — the stranger playtest falls away, journals travel only over the LAN to the local host, and the model playtest is a hard gate on seating a cohort | **Partly wired** — D219 glass screen: `RelationshipInspector` / `PieceOverlay` consume `ObservationPiece` from `projectOwnRosterObservation` (`src/orchestration/observation.ts`); `tests/glassScreen.leak.test.ts` and eslint ban `PieceState` under `src/ui/`. Still unwired: journal writer in the interactive path, harness engine in the browser (`src/app/README.md`), model agent/fork, facilitator audit/dashboard/host |
...
- **Exact gauge integers are resolved for the shipped overlay.** The piece
  overlay publishes qualitative trust, morale, and trauma labels and encodes
  aura/morale/trauma chrome from those bands only
  (`src/ui/overlays/PieceOverlay.tsx`); raw `PieceState` no longer enters
  `src/ui/` (D219 glass; `tests/glassScreen.leak.test.ts`). Testimony, rumor,
  and earned-knowledge projections remain unwired.
