# ADR implementation status (0035–0072)
...
| 0071 | Captivity and the exchange: being taken is being held | **Accepted** for D177 (owner ruling 2026-08-29); D181–D183 are accepted in principle, with the cycle-purse boundary, private ransom terms, and expectation-relative appraisal now ruled; D178–D180, D184, and D185 remain open | **Not wired** — `runMatch` returns `departedRoster`, but `sim/campaign.ts:274-296` discards it and `mergeCampaignRoster` re-creates captured pieces with reset state; the season-pool path already folds departed state. D181–D183 hold the money, private-information, role-relative self-appraisal, and piece-held-cash arithmetic; no magnitude or default-match wiring exists |
| 0072 | The ceiling on a career, and the mercy nobody buys | **Accepted** for D186 (trauma retires a career on every path) and D187 (trauma relief exists, is unearned, and cannot be purchased) — owner ruling 2026-08-29; D188 (grace rate and magnitude) and D189 (what fills a retired identity's square in the campaign path) remain open | **Not wired** — no trauma transition lowers an in-range `B_i`: `applyCaptureInjury` and `applySustainedDread` (`src/psychology/trauma.ts:10-14,17-43`) only add, while normalization only clamps malformed or out-of-range values. The trauma retirement checks exist solely on the season path (`src/orchestration/squadFielding.ts:329-334,505-512`), so the campaign path saturates at the `clampTrauma` ceiling and ends no career. No grace term, event, rate, or knob exists; the D188 anti-farming gate must be measured before any magnitude ships |
