...
| 0059 | The draft, the purse, and pieces who scout | **Proposed** — design only; magnitudes owner-owned | **Partly wired** — app and harness share one legal army plus reserve composition (`src/orchestration/squadFielding.ts:180-218`), and the information half adds the public slate and private counsel (`src/persistence/candidateSlate.ts:14-117`, `src/psychology/counsel.ts:3-139`); the economy half adds opt-in priority/purse, acceptance discount, deterministic clearing, and partial carry helpers (`src/core/draftEconomy.ts:1-252`) plus harness-only economy detectors (`sim/degeneracy.ts:410-520`); credence controls counsel disclosure only, candidate rumour appraisal state remains open, all economy magnitudes and detector thresholds are provisional §9 seeds, and no draft outcomes are persisted; `RESERVE_DEPTH=15`, cycle-one drafting, reserve/draft/economy magnitudes, and D153–D156 remain open |
